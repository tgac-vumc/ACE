##### ACNE has arrived! #####
# There are currently eight functions: acne, ploidyplotloop, ObjectsampleToTemplate, singlemodel, singleplot, getadjustedsegments, linkmutationdata, postanalysisloop, analyzegenomiclocations.
# I have also added the "multiplot" function for making summary sheets.

# acne takes a folder with rds-files (default, make sure all are segmented) or bam-files and returns plots for the most likely errors in convenient subfolders
...
# ploidyplotloop is the meat of acne, and it can be run as a separate function as well; 
...
# postanalysisloop: you've run acne, you've picked your models, you have your mutation data ... Now if you could only ...
...
acne <- function(inputdir = "./", outputdir, filetype = 'rds', binsizes, ploidies = 2, imagetype = 'pdf', method = 'RMSE', penalty = 0, cap = 12, trncname = FALSE, printsummaries = TRUE) { 
  ...
  library(QDNAseq)
	...
	if(filetype=='bam'){
	  ...
	  parameters <- data.frame(options = c("inputdir","outputdir","filetype","binsizes","ploidies","imagetype","method","penalty","cap","trncname","printsummaries"), values = c(inputdir,outputdir,filetype,binsizes,ploidies,imagetype,method,penalty,cap,trncname,printsummaries))
	  for (b in binsizes) {
	  ...
	  parameters <- data.frame(options = c("inputdir","outputdir","filetype","ploidies","imagetype","method","penalty","cap","trncname","printsummaries"), values = c(inputdir,outputdir,filetype,ploidies,imagetype,method,penalty,cap,trncname,printsummaries))
	...
	for (q in ploidies) {
  	...
  	for (a in 1:length(pd$name)) {
  		...
  		for (i in 5:100) {
  		  ...
  		  # the maximum error (e.g. 0.25 for RMSE) was added to make sure hyperamplifications (p>12) don't get ridiculous errors
  		  for (j in 1:length(segmentdata$values)) {
  		    if(method=='RMSE') {temp[j] <- min(((segmentdata$values[j]-expected)/(fraction[i-4]^penalty))^2,0.25)}
  		    else if(method=='SMRE') {temp[j] <- min(sqrt(abs((segmentdata$values[j]-expected)/(fraction[i-4]^penalty))),sqrt(0.5))}
  		    else if(method=='MAE') {temp[j] <- min(abs((segmentdata$values[j]-expected)/(fraction[i-4]^penalty)),0.5)}
...
getadjustedsegments <- function(template,cellularity = 1, ploidy = 2, standard = 1, QDNAseqobjectsample = FALSE, log=FALSE) {
  if(QDNAseqobjectsample) {template <- ObjectsampleToTemplate(template, QDNAseqobjectsample)}
  adjustedcopynumbers <- ploidy + (template$copynumbers-standard)/((1/ploidy)*cellularity*standard)
  adjustedsegments <- ploidy + (template$segments-standard)/((1/ploidy)*cellularity*standard)
  template.na <- na.exclude(template)
  segmentdata <- rle(as.vector(na.exclude(adjustedsegments)))
  copynumberdata <- as.vector(na.exclude(adjustedcopynumbers))
  Chromosome <- c()
  Start <- c()
  End <- c()
  Num_Probes <- c()
  ...
  for (i in 1:length(segmentdata$lengths)) {
    ...
    End[i] <- as.vector(template.na$end)[counter+segmentdata$lengths[i]-1]
    Num_Probes[i] <- segmentdata$lengths[i]
    if(log==TRUE) {Segment_Mean[i] <- log2(segmentdata$values[i])}
    if(log==FALSE) {
      Segment_Mean[i] <- segmentdata$values[i]
      Segment_Mean2[i] <- mean(copynumberdata[counter:(counter+segmentdata$lengths[i]-1)])
      Segment_SE[i] <- sd(copynumberdata[counter:(counter+segmentdata$lengths[i]-1)])/sqrt(Num_Probes[i])
...
linkmutationdata <- function(filename, segmentdf, cellularity = 1,chrindex=1,posindex=2,freqindex=3, append=TRUE, outputdir){
  ...
  if (!inherits(inputvcf, "try-error")) {
    ...
    if (missing(outputdir)) {
      fn <- gsub(".csv","_acne.csv",filename)
      fn <- gsub(".tsv","_acne.tsv",fn)
      fn <- gsub(".txt","_acne.txt",fn)
      fn <- gsub(".xls","_acne.xls",fn)
